...
function serialEvent() {
  let bytes = serial.readBytes();
  if (!bytes || bytes.length < 10) return;

  for (let i = 0; i <= bytes.length - 10; i++) {
    if (bytes[i] === 0xAA) {
      let data = bytes.slice(i + 1, i + 9);
      let checksum = bytes[i + 9];
      let calcChecksum = data.reduce((sum, b) => sum + b, 0) % 256;

      if (checksum === calcChecksum) {
        let dv = new DataView(Uint8Array.from(data).buffer);
        microBitX = dv.getInt16(0) + windowWidth / 2;
        microBitY = dv.getInt16(2) + windowHeight / 2;
        microBitAState = dv.getUint8(4) === 1;
        microBitBState = dv.getUint8(5) === 1;
        break;
      }
    }
  }
...
https://editor.p5js.org/hannah26/sketches/FL4-xKMwz
